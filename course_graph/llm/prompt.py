...
class ParserPrompt(ABC):
    """ 使用大模型解析文档相关提示词
    """

    @staticmethod
    def get_ocr_aided_prompt(text: str):
        ...
        Returns:
            str: prompt输出
        """
        return f"""你是一个大语言模型, 专门用于帮助用户纠正从图片中提取的OCR文字错误。你的任务是输出经过纠正后的准确文字。遵循以下准则:
    ...
    @staticmethod
    def get_directory_prompt(content: str) -> str:
        """ 使用大模型纠正整理目录, 获取相应提示词
    ...
    @staticmethod
    def get_outline_prompt(directory_list: list) -> str:
        """ 使用大模型为目录添加层级
