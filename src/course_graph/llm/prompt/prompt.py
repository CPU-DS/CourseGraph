...
class ExamplePromptGenerator(PromptGenerator):
    ...
    def get_ner_prompt(self, content: str) -> tuple[str, str]:
        ...
        return json.dumps(prompt, indent=4, ensure_ascii=False), "你是专门进行知识点实体抽取的专家"
    ...
    def get_re_prompt(self, content: str,
        ...
        prompt = {
            "instruction":
                "请根据提供的中心知识点和已有文本片段, 一步步思考, 寻找与之相关联的知识点并判断二者之间的关系, 如果存在关系但不在所指定的关系范围relations中, 则不返回。头尾实体不应该相同。返回为你的思考和关系三元组, 格式为 ```json\n[{\"head\": \"\", \"relation\": \"\", \"tail\": \"\"}]\n```",
            "relations": ONTOLOGY['relations'],
            "examples": examples,
            "input": f"中心实体列表为: {entities}, 文本片段为: '{content}'"
        }
        return json.dumps(prompt, indent=4, ensure_ascii=False), "你是专门进行知识点关系判别的专家"
    ...
    def get_ae_prompt(self, content: str,
        ...
        return json.dumps(prompt, indent=4, ensure_ascii=False), "你是专门进行知识点属性抽取的专家"
